...
@Slf4j
@Component
public class Base64Detector extends BaseDetector {

    /*
     * Base64 thường xuất hiện trong dấu nháy, ví dụ: "cG93ZXJzaGVsbA=="
     */
    ...
    public List<Finding> detect(File file) {
        ...
        try {
            ...
            for (int i = 0; i < lines.size(); i++) {
                ...
                List<String> candidates = extractBase64Strings(rawLine);

                for (String encoded : candidates) {

                    if (!isValidBase64(encoded)) {

                        log.debug("Invalid base64: {}", encoded);
                    ...
                    if (containsDangerousPayload(decoded)) {
                        ...
                        findings.add(
                                Finding.builder()
                                        ...
                                        .line(lineNumber)
                                        .type(FindingType.BASE64_OBFUSCATION)
                                        .severity(SeverityLevel.HIGH)
                                        .matchedKeyword(encoded)
                                        .codeSnippet(rawLine)
            ...
            log.error("Base64Detector error", e);
        }

        return findings;
    }

    private List<String> extractBase64Strings(String content) {
    ...
    private boolean containsDangerousPayload(String decoded) {

        String lower = decoded.toLowerCase();

        return DANGEROUS_KEYWORDS.stream().anyMatch(lower::contains);
    }
}
